...
@section('contain')
    <div class="row page-titles" style="box-shadow: 0px 0px 14px 3px #cfcfcf; background: linear-gradient(45deg, rgb(241, 234, 234), transparent);border: 1px solid #cdcdcd;">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">Reward Management</h3>
        </div>
        <div class="col-md-7 align-self-center text-end">
            <div class="d-flex justify-content-end align-items-center">
                ...
                {{-- <a href="{{route('reward.form')}}"><button type="button"
                    ...
                    <tbody>
                        ...
                        @foreach ($rewards as $reward)
                            <tr>
                                ...
                                <td>
                                    @foreach ($reward->values as $value)
                                        <span class="badge badge-primary waves-effect waves-dark">{{ $value }}</span>
                                        {{-- <span class="badge badge-danger">Primary</span> --}}
                                    @endforeach
                                </td>
                                <td class="text-canter">
                                    <a title="Edit" href="{{ route('reward.edit', $reward->id) }}"><i
                                            class="far fa-edit" style="font-size:20px"></i></a>

                                    {{-- <a title="Delete" type="button" onclick="swalll({{ $reward->id }})"><i class="fa fa-trash "
                                            aria-hidden="true"></i></a>
                                    <form id="delete-form-{{ $reward->id }}"
                                        action="{{ route('reward.delete', $reward->id) }}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form> --}}
