    ...
    public function index()
    {
        ...
        foreach ($ids as $id) {
            ...
            if (isset($apiData) && !empty($apiData)) {
                $foundItems[] = [
                    'id' => $apiData['BenfId'] ?? null, // Use null coalescing to avoid undefined index
                    'uuid' => $apiData['UuId'] ?? null,
                    'filled_da_form_id' => $apiData['FormId'] ?? null,
                    'da_cnic' => $apiData['Cnic'] ?? null,
                    'da_occupant_name' => $apiData['BenfName'] ?? null,
                    'gender' => $apiData['Gender'] ?? null,
                    'district' => $apiData['District'] ?? null,
                    'tehsil' => $apiData['Tehsil'] ?? null,
                    'union_council' => $apiData['Uc'] ?? null,
                    'deh' => $apiData['Dehat'] ?? null,
                    'widows' => $apiData['Vul01'] ?? null,
                    'women_with_disable_husband' => $apiData['Vul02'] ?? null,
                    'divorced_abandoned_unmarried_older_dependent_on_others' => $apiData['Vul03'] ?? null,
                    'people_with_disability_physically_or_mentally' => $apiData['Vul04'] ?? null,
                    'unaccompained_minors_i_e_orphans' => $apiData['Vul05'] ?? null,
                    'unaccompained_elders_over_the_age_of_60' => $apiData['Vul06'] ?? null,
                ];
            }
        }
